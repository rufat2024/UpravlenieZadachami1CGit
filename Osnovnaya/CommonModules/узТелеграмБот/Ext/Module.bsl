...
Функция ПолучитьНовыеСообщения(MaxMessageID = "", Ответ = Неопределено, НеПолучатьПовторно = Истина) Экспорт
	...
	Если НЕ ВыполнитьЗапросHTTP(НастройкиБота, "getUpdates", ПараметрыЗапроса, Ответ, Ложь) Тогда
	...
	Для Каждого Update Из ОтветСоответствие["result"] Цикл
		...
		Если Message <> Неопределено Тогда
			...
			пПользовательПредставление = "" + msgChat["last_name"] + " " + msgChat["first_name"];
			ДанныеСообщения.Вставить("ПользовательПредставление", пПользовательПредставление);
...
//*rufat@, 2025-03-04 10:07:55 Учет времени ОВ
//Функция ПолучитьФайл(ФайлИД, НастройкиБота = Неопределено) Экспорт
// -------- заменено на:
Функция Руф_ПолучитьФайл(ФайлИД, НастройкиБота = Неопределено) Экспорт
//*/rufat@, 2025-03-04 10:07:55
